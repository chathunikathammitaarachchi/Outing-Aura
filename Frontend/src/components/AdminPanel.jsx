...
const AddFoodPackage = () => {
  ...
  const [newPackages, setNewPackages] = useState([
    { name: "", description: "", price: "", category: "Food", imageUrl: "", image: null },
  ]);
  ...
  const fetchPackages = async () => {
    ...
    try {
      ...
      const foodPackages = res.data.filter(pkg => pkg.category === "Food");
      setPackages(foodPackages);
    } catch (error) {
  ...
  const handleAddOrUpdatePackages = async () => {
    try {
      ...
      for (const formData of formDataList) {
        await axios.post("http://localhost:8080/api/package", formData);
      }

      fetchPackages();
      setNewPackages([{ name: "", description: "", price: "", category: "Food", imageUrl: "", image: null }]);
    } catch (error) {
  ...
  const handleDeletePackage = async (_id) => {
    if (window.confirm("Are you sure you want to delete this package?")) {
      try {
        await axios.delete(`http://localhost:8080/api/package/${_id}`);
        fetchPackages();
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        {editingPackage ? "Edit Package" : "Add Multiple Food Packages"}
      </h2>
