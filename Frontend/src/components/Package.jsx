...
const Package = () => {
  ...
  useEffect(() => {
    ...
    if (location.state && location.state.selectedEvent) {
      ...
      setSelectedFilters([eventType, "food"]);
    }
  }, [location.state]);

  const fetchPackages = async () => {
    try {
      const foodPackagesRes = await axios.get("http://localhost:8080/api/package");
  ...
  pkg.category &&
  pkg.category.toLowerCase().includes("food") &&
  (
    (pkg.name && pkg.name.toLowerCase().includes(eventType.toLowerCase())) ||
      ...
      setFilteredPackages(
        packages.filter((pkg) =>
          selectedFilters.every(
            (filter) =>
              (pkg.category && pkg.category.toLowerCase().includes(filter.toLowerCase())) ||
              (pkg.name && pkg.name.toLowerCase().includes(filter.toLowerCase()))
          )
        )
      );
    }
    ...
    <div>
      ...
      <div
        style={{
          ...
          alignItems: "center",
          color: "black",
          textShadow: "2px 2px 4px rgba(7, 6, 6, 0.5)",
        }}
      >
        <h1 style={{ fontSize: "36px", fontWeight: "bold" }}>Our Services</h1>
      </div>
      ...
      <h1 style={{ textAlign: "center", margin: "80px 0", fontSize: "28px", color: "black" }}>
        {location.state && location.state.selectedEvent
          ? `Food Packages for ${location.state.selectedEvent}`
          : "Available Packages"}
      </h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "20px", marginTop: "40px" }}>
        {filteredPackages.length > 0 ? (
          ...
          <div style={{ textAlign: "center", fontSize: "18px", color: "#555" }}>
            <p>No food packages available for the selected event type.</p>
          </div>
        )}
      </div>
    </div>
